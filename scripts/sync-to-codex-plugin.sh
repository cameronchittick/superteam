...
#
# Sync this superteam checkout → prime-radiant-inc/openai-codex-plugins.
...
# plugins/superteam/ when absent, then copies the tracked plugin files from
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  SYNC_BRANCH="bootstrap/superteam-${UPSTREAM_SHORT}-${TIMESTAMP}"
else
  SYNC_BRANCH="sync/superteam-${UPSTREAM_SHORT}-${TIMESTAMP}"
fi
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  echo "Mode:     BOOTSTRAP (creating plugins/superteam/ when absent)"
fi
echo ""
...
if [[ $BOOTSTRAP -eq 1 ]]; then
  COMMIT_TITLE="bootstrap superteam v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Initial bootstrap of the superteam plugin from upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).

Creates \`plugins/superteam/\` by copying the tracked plugin files from upstream, including \`.codex-plugin/plugin.json\`, \`assets/\`, and \`hooks/\`.
...
This is a one-time bootstrap. Subsequent syncs will be normal (non-bootstrap) runs using the same tracked upstream plugin files."
  ...
  COMMIT_TITLE="sync superteam v$UPSTREAM_VERSION from upstream main @ $UPSTREAM_SHORT"
  PR_BODY="Automated sync from superteam upstream \`main\` @ \`$UPSTREAM_SHORT\` (v$UPSTREAM_VERSION).
