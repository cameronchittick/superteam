#!/usr/bin/env bash
#
# Package the Superteam Codex plugin as a rootless archive for portal upload.
#
...
Options:
  --output PATH            Write archive to PATH.
                           Default: ../_tmp/sup-codex-packaging/superteam-VERSION.zip
  ...
  --metadata-source PATH   Prior official package directory, .zip, or .tar.gz used to
                           ...
                           Default: ../_tmp/sup-codex-packaging/superteam,
                           falling back to superteam.zip, then superteam.tar.gz
...
if [[ -z "$METADATA_SOURCE" ]]; then
  if [[ -d "$REPO_ROOT/../_tmp/sup-codex-packaging/superteam" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/sup-codex-packaging/superteam"
  elif [[ -f "$REPO_ROOT/../_tmp/sup-codex-packaging/superteam.zip" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/sup-codex-packaging/superteam.zip"
  elif [[ -f "$REPO_ROOT/../_tmp/sup-codex-packaging/superteam.tar.gz" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/sup-codex-packaging/superteam.tar.gz"
  else
    die "no metadata source found; pass --metadata-source <prior package dir, zip, or tar.gz>"
  fi
fi

WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/superteam-codex-package.XXXXXX")"
...
if [[ -z "$OUTPUT" ]]; then
  case "$FORMAT" in
    zip)
      OUTPUT="$REPO_ROOT/../_tmp/sup-codex-packaging/superteam-$VERSION.zip"
      ;;
    tar.gz)
      OUTPUT="$REPO_ROOT/../_tmp/sup-codex-packaging/superteam-$VERSION.tar.gz"
      ;;
  esac
fi
...
unexpected_paths="$(
  printf '%s\n' "$archive_paths" |
    grep -E '(^superteam/|^\.agents/|^hooks/|package\.json$|^\.git|^\.pytest_cache|^\.ruff_cache|^scripts/|^tests/|^docs/|^evals/|^lib/|^\.claude|^\.cursor|^\.kimi|^\.opencode|^\.pi|^AGENTS\.md$|^CLAUDE\.md$|^GEMINI\.md$|^RELEASE-NOTES\.md$|^CHANGELOG\.md$)' || true
)"
