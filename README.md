...
Where each task runs — an IC on trunk, on a branch in the lead's checkout, in a worktree, or in a provisioned lane — and which seats sit (skeptic, reviewer, how many ICs) follow the lead's sizing of the brief — files touched, other writers, cost of a mistake, design choice, independent pieces; see [docs/isolation-tiers.md](docs/isolation-tiers.md).
