...
EOF
    ...
    cat > "$repo/assets/superteam-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/superteam/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/superteam/.fixture-keep"
    cat > "$repo/plugins/superteam/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/superteam/.fixture-keep
    git -C "$repo" add plugins/superteam/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/superteam/skills/example/agents"

    cat > "$repo/plugins/superteam/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/superteam/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/superteam/.codex-plugin" \
        "$repo/plugins/superteam/.private-journal" \
        "$repo/plugins/superteam/assets" \
        "$repo/plugins/superteam/hooks" \
        "$repo/plugins/superteam/skills/example/agents" \
        "$repo/plugins/superteam/skills/example"

    cat > "$repo/plugins/superteam/.codex-plugin/plugin.json" <<EOF
{
  "name": "superteam",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/superteam/assets/superteam-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/superteam/assets/app-icon.png"

    cat > "$repo/plugins/superteam/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/superteam/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/superteam/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/superteam/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
    chmod +x "$repo/plugins/superteam/hooks/session-start" "$repo/plugins/superteam/hooks/session-start-codex" "$repo/plugins/superteam/hooks/run-hook.cmd"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/superteam/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/superteam/.codex-plugin/plugin.json \
        plugins/superteam/assets/app-icon.png \
        plugins/superteam/assets/superteam-small.svg \
        plugins/superteam/hooks/hooks-codex.json \
        plugins/superteam/hooks/run-hook.cmd \
        plugins/superteam/hooks/session-start \
        plugins/superteam/hooks/session-start-codex \
        plugins/superteam/skills/example/agents/openai.yaml \
        plugins/superteam/skills/example/SKILL.md \
        plugins/superteam/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/superteam/.kimi-plugin" \
        "$repo/plugins/superteam/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/superteam/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/superteam/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/superteam/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/superteam/.fixture-keep \
        plugins/superteam/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/superteam/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/superteam/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/superteam-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (superteam-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/superteam-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/superteam/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/superteam" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/superteam-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/superteam'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/superteam-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/superteam-*" "Clean no-op local apply does not create sync branch in destination checkout"
