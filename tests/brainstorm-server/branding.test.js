/**
 * Tests for the visual companion's Superteam/Prime Radiant branding.
 */
...
const ASSET_URL = 'https://primeradiant.com/brand/superteam-visual-brainstorming-logo.png';
...
function assertBrandedWithLogo(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Superteam v${version}`),
    'branding text should include dynamic package version'
  );
  assert(
    !html.includes(`Superteam v${version} by`),
    'branding text should not include "by" when the logo is visible'
  );
  assert(
    /<img class="brand-logo"[^>]*>\s*<span class="brand-copy">Superteam v/.test(html),
    'visible logo should appear before the Superteam version text'
  );
  assert(
...
function assertBrandedFallbackText(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Prime Radiant Superteam v${version}`),
    'disabled telemetry should keep plain text Prime Radiant/Superteam branding'
  );
}
...
async function main() {
  ...
  await test('SUPERTEAM_DISABLE_TELEMETRY=true omits remote image but keeps local branding', async () => {
    const port = 3453;
    const dir = '/tmp/brainstorm-branding-disabled';
    await withServer({ port, dir, env: { SUPERTEAM_DISABLE_TELEMETRY: 'true' } }, async () => {
      writeFragment(dir);
      await sleep(300);
      const html = await fetchHtml(port);
  ...
  await test('SUPERTEAM_DISABLE_TELEMETRY=yes also omits the remote image on the waiting screen', async () => {
    const port = 3454;
    const dir = '/tmp/brainstorm-branding-disabled-waiting';
    await withServer({ port, dir, env: { SUPERTEAM_DISABLE_TELEMETRY: 'yes' } }, async () => {
      const html = await fetchHtml(port);
      assertBrandedFallbackText(html);
      assert(!html.includes(ASSET_URL), 'disabled telemetry should omit the remote image');
    });
