/**
 * Tests for the visual companion's Superteam branding: plain local text, no remote assets.
 */
...
function createPackagedServerFixture(version) {
  const root = fs.mkdtempSync(path.join('/tmp', 'superteam-packaged-server-'));
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'superteam', version }, null, 2)
  );
  return {
    root,
...
function assertPlainBranding(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`<div class="brand"><span class="brand-copy">Superteam v${version}</span></div>`),
    'branding should be plain "Superteam v<version>" text'
  );
  assert(!html.includes(`Superteam v${version} by`), 'branding text should not include "by"');
  assert(!html.includes('<img'), 'branding must not load any image');
  assert(!html.includes('href'), 'branding must not link anywhere');
  assert(!/primeradiant/i.test(html), 'branding must not reference primeradiant');
  assert(!/Prime Radiant/i.test(html), 'branding must not reference Prime Radiant');
  assert(!html.includes('brand-logo'), 'no logo CSS should remain');
  assert(
    /\.brand-copy\s*\{[^}]*text-overflow:\s*ellipsis/i.test(html),
    'version text should still truncate with an ellipsis'
  );
  assert(
    /\.brand\s*\{[^}]*display:\s*flex/i.test(html),
    'brand wrapper should still be a flex row'
  );
  assert(
...
async function main() {
  ...
  await test('framed screens render plain versioned Superteam text', async () => {
    const port = 3451;
    ...
    await withServer({ port, dir }, async () => {
      ...
      assertPlainBranding(html);
      assertFramedScreenUsesBrandHeader(html);
      assertHeaderAvoidsNarrowOverlap(html);
    });
  });

  await test('waiting screen renders plain versioned Superteam text', async () => {
    const port = 3452;
  ...
  await test('packaged Codex plugin reads version from .codex-plugin manifest', async () => {
    ...
    try {
      await withServer({ port, dir, serverPath: fixture.serverPath }, async () => {
        ...
        await sleep(300);
        const html = await fetchHtml(port);
        assertPlainBranding(html, packagedVersion);
        assert(!html.includes('Superteam vunknown'), 'packaged plugin should not fall back to unknown version');
      });
    } finally {
  ...
  await test('telemetry env vars have no effect on output', async () => {
    const dir = '/tmp/brainstorm-branding-env';
    const render = async (port, env) => {
      let html;
      await withServer({ port, dir, env }, async () => {
        writeFragment(dir);
        await sleep(300);
        html = await fetchHtml(port);
      });
      return html;
    };
    const baseline = await render(3453, {});
    assert.strictEqual(await render(3454, { SUPERTEAM_DISABLE_TELEMETRY: 'true' }), baseline);
    assert.strictEqual(await render(3455, { DISABLE_TELEMETRY: 'true' }), baseline);
  });
