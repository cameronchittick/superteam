...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/superteam-tests/${TIMESTAMP}/explicit-skill-requests/multiturn"
mkdir -p "$OUTPUT_DIR"

# Create project directory (conversation is cwd-based)
PROJECT_DIR="$OUTPUT_DIR/project"
mkdir -p "$PROJECT_DIR/docs/superteam/plans"
...
cat > "$PROJECT_DIR/docs/superteam/plans/auth-system.md" << 'EOF'
...
claude -p "Good analysis. I've already written the plan to docs/superteam/plans/auth-system.md. Now I'm ready to implement. What are my options for execution?" \
    --continue \
    --plugin-dir "$PLUGIN_DIR" \
    --dangerously-skip-permissions \
    --max-turns 2 \
...
# Turn 3: The critical test - ask for superteam-driven-development
echo ">>> Turn 3: Requesting superteam-driven-development..."
...
if grep -q '"name":"Skill"' "$TURN3_LOG" && grep -qE "$SKILL_PATTERN" "$TURN3_LOG"; then
    echo "PASS: Skill 'superteam-driven-development' was triggered in Turn 3"
    TRIGGERED=true
else
    echo "FAIL: Skill 'superteam-driven-development' was NOT triggered in Turn 3"
    TRIGGERED=false
fi
