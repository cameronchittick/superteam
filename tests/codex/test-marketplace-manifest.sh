...
assert_equal(marketplace.get("name"), "superteam-dev", "marketplace name")
assert_equal(
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "superteam"]
assert_equal(len(matching_plugins), 1, "superteam plugin entry count")
