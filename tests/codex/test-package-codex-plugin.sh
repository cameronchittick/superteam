...
archive="$TEST_ROOT/superteam"
tar_archive="$TEST_ROOT/superteam.tar.gz"
...
unexpected_pattern='(^superteam/|^\.agents/|^hooks/|package\.json$|^\.git|^\.pytest_cache|^\.ruff_cache|^scripts/|^tests/|^docs/|^evals/|^lib/|^\.claude|^\.cursor|^\.kimi|^\.opencode|^\.pi|^AGENTS\.md$|^CLAUDE\.md$|^GEMINI\.md$|^RELEASE-NOTES\.md$|^CHANGELOG\.md$)'
...
assert_contains "$archive_paths" "assets/superteam-small.svg" "archive includes composer icon"
...
assert_equals "$manifest_summary" "superteam	$expected_version	./skills/	$source_hooks" "archive manifest preserves source hooks"
...
if [[ -x "$extracted/skills/superteam-driven-development/scripts/task-brief" ]]; then
...
tar_task_brief_mode="$(tar -tzvf "$tar_archive" skills/superteam-driven-development/scripts/task-brief | awk '{print $1}')"
...
archive_from_tar_source="$TEST_ROOT/superteam-from-tar-source.zip"
archive_from_zip_source="$TEST_ROOT/superteam-from-zip-source.zip"
(
  cd "$metadata_source"
