...
# 1. Create in superteam location (lowest priority)
mkdir -p "$SUPERTEAM_SKILLS_DIR/priority-test"
cat > "$SUPERTEAM_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Superteam version of priority test skill
---
# Priority Test Skill (Superteam Version)

This is the SUPERTEAM version of the priority test skill.

PRIORITY_MARKER_SUPERTEAM_VERSION
EOF
...
if [ -f "$SUPERTEAM_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Superteam version exists"
else
    echo "  [FAIL] Superteam version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled superteam priority
echo ""
echo "Test 2: Documenting personal vs superteam priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superteam version instead of personal version"

# Test 3: Document project vs bundled superteam priority
echo ""
echo "Test 3: Documenting project vs personal/superteam priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superteam version instead of project version"

# Test 4: Test a non-colliding bundled superteam skill is still available
echo ""
echo "Test 4: Testing non-colliding superteam skill remains available..."

mkdir -p "$SUPERTEAM_SKILLS_DIR/superteam-only-test"
cat > "$SUPERTEAM_SKILLS_DIR/superteam-only-test/SKILL.md" <<'EOF'
---
name: superteam-only-test
description: Superteam-only priority test skill
---
# Superteam Only Test Skill

PRIORITY_MARKER_SUPERTEAM_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"superteam-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SUPERTEAM_ONLY_VERSION" "Non-colliding superteam skill is still registered"
