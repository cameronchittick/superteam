#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the superteam plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/superteam.js"
...
skill_count=$(find "$SUPERTEAM_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $SUPERTEAM_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-superteam skill exists (critical for bootstrap)
echo "Test 3: Checking using-superteam skill (required for bootstrap)..."
if [ -f "$SUPERTEAM_SKILLS_DIR/using-superteam/SKILL.md" ]; then
    echo "  [PASS] using-superteam skill exists"
else
    echo "  [FAIL] using-superteam skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$SUPERTEAM_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/superteam/' "$SUPERTEAM_PLUGIN_FILE"; then
