...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERTEAM_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$SUPERTEAM_SKILLS_DIR/using-superteam/SKILL.md" "$TEST_HOME/using-superteam.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERTEAM_PLUGIN_FILE" missing
}
