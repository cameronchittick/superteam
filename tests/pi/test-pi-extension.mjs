...
const extensionPath = resolve(repoRoot, '.pi/extensions/superteam.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-superteam/references/pi-tools.md');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'superteam');
  assert.ok(pkg.keywords.includes('pi-package'));
  assert.deepEqual(pkg.pi.skills, ['./skills']);
  assert.deepEqual(pkg.pi.extensions, ['./.pi/extensions/superteam.ts']);
});
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have superteam/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have superteam/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have superteam/);
