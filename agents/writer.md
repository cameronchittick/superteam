...
You usually do not have the Task tools in a worktree; the lead claims and completes your task on the list from your report. If `TaskUpdate` is in your tool list anyway, claim it (owner=<your name>, status=in_progress) and complete it only after your `Verified:` line is written, never with failing tests or partial work; do not claim other tasks unless the lead says so — lead-crafted briefs are load-bearing.
