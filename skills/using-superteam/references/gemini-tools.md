...
Skills dispatch with `Subagent (general-purpose):` and either reference a prompt-template file (e.g., `superteam:superteam-driven-development`'s `./implementer-prompt.md`) or supply an inline prompt. On Gemini CLI:
...
| References `superteam:requesting-code-review`'s `./standards-reviewer.md` + `./spec-reviewer.md` | Two `invoke_agent` calls with `agent_name: "generalist"`, one per filled review template, in the same turn |
