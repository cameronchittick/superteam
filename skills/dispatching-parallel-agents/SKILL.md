...
- **Agents will edit files:** `subagent_type: "superteam:implementer"` with `isolation: "worktree"  # parallel writers = worktree tier; see docs/isolation-tiers.md` on each call so they cannot overwrite each other. Each works on its own `worktree-<name>` branch; you merge after review (Step 4).
