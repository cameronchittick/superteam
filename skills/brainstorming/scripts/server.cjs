...
const SUPERTEAM_VERSION = readSuperteamVersion();
const SUPERTEAM_BRAND_IMAGE_URL = 'https://primeradiant.com/brand/superteam-visual-brainstorming-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'SUPERTEAM_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const SUPERTEAM_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(SUPERTEAM_VERSION);
  const text = SUPERTEAM_TELEMETRY_DISABLED
    ? 'Prime Radiant Superteam v' + version
    : 'Superteam v' + version;
  const logo = SUPERTEAM_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + SUPERTEAM_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(SUPERTEAM_VERSION) + '" alt="Prime Radiant" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a>' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
