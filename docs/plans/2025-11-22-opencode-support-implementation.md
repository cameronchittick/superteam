...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superteam:executing-plans to implement this plan task-by-task.

**Goal:** Add full superteam support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/superteam-codex` (lines 40-74)
...
- Reference: `.codex/superteam-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'superteam' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superteam-codex` (lines 212-280)
 ...
 * (personal skills override superteam skills).
 *
 * @param {string} skillName - Name like "superteam:brainstorming" or "my-skill"
 * @param {string} superteamDir - Path to superteam skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superteamDir, personalDir) {
    // Strip superteam: prefix if present
    const forceSuperteam = skillName.startsWith('superteam:');
    const actualSkillName = forceSuperteam ? skillName.replace(/^superteam:/, '') : skillName;

    // Try personal skills first (unless explicitly superteam:)
    if (!forceSuperteam && personalDir) {
    ...
    // Try superteam skills
    if (superteamDir) {
        const superteamPath = path.join(superteamDir, actualSkillName);
        const superteamSkillFile = path.join(superteamPath, 'SKILL.md');
        if (fs.existsSync(superteamSkillFile)) {
            return {
                skillFile: superteamSkillFile,
                sourceType: 'superteam',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superteam-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/superteam-codex` (add import at top)
...
Run: `node -c .codex/superteam-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/superteam-codex
...
**Files:**
- Modify: `.codex/superteam-codex` (lines 40-74)
...
Run: `.codex/superteam-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/superteam-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/superteam-codex` (lines 16-38, approximately)
...
Run: `.codex/superteam-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/superteam.js`
...
/**
 * Superteam plugin for OpenCode.ai
 *
...
const superteamSkillsDir = path.join(homeDir, '.config/opencode/superteam/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SuperteamPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/superteam.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/superteam.js
...
**Files:**
- Modify: `.opencode/plugin/superteam.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "superteam:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > superteam)
...
Run: `node -c .opencode/plugin/superteam.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the superteam and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const superteamSkills = skillsCore.findSkillsInDir(
            superteamSkillsDir,
            'superteam',
            3
          );
          ...
          const allSkills = [...personalSkills, ...superteamSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install superteam skills to ~/.config/opencode/superteam/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'superteam:';
...
```javascript
    'session.started': async () => {
      // Read using-superteam skill content
      const usingSuperteamPath = skillsCore.resolveSkillPath(
        'using-superteam',
        superteamSkillsDir,
        personalSkillsDir
      );

      let usingSuperteamContent = '';
      if (usingSuperteamPath) {
        const fullContent = fs.readFileSync(usingSuperteamPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSuperteamContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Superteam skills: \`superteam:skill-name\` (from ~/.config/opencode/superteam/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/superteam')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/superteam && git pull` to update superteam.' :
        '';
...
**Below is the full content of your 'superteam:using-superteam' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingSuperteamContent}
...
```markdown
# Installing Superteam for OpenCode
...
### 1. Install Superteam Skills

```bash
# Clone superteam skills to OpenCode config directory
mkdir -p ~/.config/opencode/superteam
git clone https://github.com/obra/superpowers.git ~/.config/opencode/superteam
```

### 2. Install the Plugin

The plugin is included in the superteam repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/superteam/.opencode/plugin/superteam.js`
...
ln -s ~/.config/opencode/superteam/.opencode/plugin/superteam.js .opencode/plugin/superteam.js
```
...
```
use use_skill tool with skill_name: "superteam:brainstorming"
```
...
Personal skills override superteam skills with the same name.

## Updating

```bash
cd ~/.config/opencode/superteam
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/superteam/.opencode/plugin/superteam.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/superteam/skills`
...
Superteam works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/superteam-codex use-skill superteam:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/superteam-codex bootstrap | head -30`
...
ls -l .opencode/plugin/superteam.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/superteam.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/superteam-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/superteam-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/superteam.js` created with tools and hooks
