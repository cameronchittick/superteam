# Superteam for Kimi Code

Complete guide for using Superteam with [Kimi Code](https://github.com/MoonshotAI/kimi-code).
...
2. Loads `using-superteam` at session start through `sessionStart.skill`.
3. Provides Kimi-specific tool mapping through `skillInstructions`.

Kimi Code reads Superteam skills from this repository. There are no copied skills, symlinks, hooks, or extra runtime dependencies.
...
Select Superteam and update it from there. Start a fresh session with `/new` after updating.

## Troubleshooting

### Plugin not loading

1. Run `/plugins info superteam` and check diagnostics.
...
Kimi Code installs the latest GitHub release for a bare repository URL when one exists. To test unreleased changes before the next Superteam release, install the branch explicitly:
...
1. Confirm `/plugins info superteam` shows the plugin enabled.
