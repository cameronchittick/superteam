...
SDD's durable-progress workspace (`.superteam/sdd/`, introduced v6.0.0/v6.0.3) has
...
The workspace becomes `.superteam/sdd/<plan-slug>/`, where `<plan-slug>` is
...
- `sdd-workspace PLAN_FILE` — resolves and creates
  `<repo-root>/.superteam/sdd/<plan-slug>/`, maintains the self-ignoring
  `.gitignore` at `.superteam/sdd/.gitignore` (parent level, content `*`),
...
```
# SDD ledger — plan: docs/superteam/plans/<plan-file>.md
```
...
`superteam:finishing-a-development-branch` — the controller deletes its
...
`WAVE1-HANDOFF.md`) live directly under `.superteam/sdd/` untouched by any
plan's cleanup.
...
- No git-level guards against committing `.superteam/` beyond the existing
  parent `.gitignore`.
...
- `sdd-workspace PLAN` prints `<root>/.superteam/sdd/<slug>` and creates it;
...
Results land in `docs/superteam/specs/2026-07-06-sdd-plan-scoped-workspace-eval-results.md`
