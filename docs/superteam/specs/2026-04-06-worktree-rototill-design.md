...
Superteam is opinionated about worktree management — specific paths (`.worktrees/<branch>`), specific commands (`git worktree add`), specific cleanup (`git worktree remove`). Meanwhile, Claude Code, Codex App, Gemini CLI, and Cursor all provide native worktree support with their own paths, lifecycle management, and cleanup.
...
For harnesses without native support (Codex CLI, OpenCode, Copilot standalone), superteam fills a real gap. The skill shouldn't go away — it should get out of the way when native support exists.
...
- `.superteam-session.json` metadata tracking (interesting PR #997 idea, not needed for v1)
...
Whoever creates the worktree owns its cleanup. If the harness created it, superteam doesn't touch it. If superteam created it (via git fallback), superteam cleans it up. The heuristic: if the worktree lives under `.worktrees/` or `worktrees/`, superteam owns it. Anything else (`.claude/worktrees/`, `~/.codex/worktrees/`, `.gemini/worktrees/`, or old user-global Superteam paths) belongs to the harness or user and is left alone.
...
No interactive directory selection prompt. Old user-global Superteam worktree paths are not detected or offered; new manual worktrees are project-local unless the user explicitly specifies another location.
...
git worktree remove "$WORKTREE_PATH"  # only if superteam owns it
...
**Option 4 (Discard):** Require typed "discard" confirmation. Then remove worktree (if superteam owns it), force-delete branch.
...
if worktree path is under .worktrees/ or worktrees/:
    # Superteam created it — we own cleanup
...
The `.worktrees/` or `worktrees/` = ours, anything else = hands off` heuristic works for every current harness. If a future harness adopts one of those project-local directories as its convention, we'd have a false positive (superteam tries to clean up a harness-owned worktree). Similarly, if a user manually runs `git worktree add .worktrees/experiment` without superteam, we'd incorrectly claim ownership. Both are low risk — every harness uses branded paths, and manual `.worktrees/` creation is unlikely — but worth noting.
