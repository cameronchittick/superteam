# Lift drill into superteam as `evals/` — design
...
Drill is already the *de facto* eval harness for superteam. The PRI-1397 commit series in the drill repo lifted ~22 superteam bash tests into drill scenarios, and the most recent superteam commit (`a2292c5`) explicitly removed a redundant bash test with the message *"replaced by drill behavioral coverage"*. Migration momentum exists; this spec completes it.

This work moves drill into superteam under `evals/`, deletes the redundant bash tests after per-file verification of drill scenario coverage, and updates docs so contributors land on the new structure.

## Goals

1. `evals/` is the canonical eval harness in superteam — full drill source, scenarios, fixtures, prompts, backend configs, and tests.
2. Bash tests in `superteam/tests/` that have been individually verified as 100% covered by drill scenarios are deleted; the rest are preserved.
...
- **Drill repo archival.** This PR does not touch `obra/drill`. After merge, the drill repo is archived manually (read-only on GitHub, README pointer to `obra/superteam/evals/`).
...
| Drill's own pytest suite | Subagent runs `cd evals && uv run pytest` after the path-default change. Drill ships its own pytest suite at `evals/tests/` including `test_backend.py` which exercises `SUPERTEAM_ROOT` env-var behavior — these tests must update to match the helper and continue to pass. |
| Reference scrubbing after deletion | Subagent greps the entire superteam tree (excluding `node_modules/`, `.venv/`, and `evals/`) for references to deleted bash test paths. Search targets: `docs/`, `docs/superteam/plans/`, `RELEASE-NOTES.md`, `CLAUDE.md`, `GEMINI.md`, `AGENTS.md`, `README.md`, `.github/`, `scripts/`, `.opencode/INSTALL.md`, `.codex-plugin/INSTALL.md`, `lefthook.yml`. Any hit is either updated or surfaces a missed dependency. |
| Path defaults change (`SUPERTEAM_ROOT` default) | Subagent runs at least one cheap drill scenario after the path changes (e.g., `triggering-test-driven-development`) and confirms it still passes. Real validation, not just code review. |
...
**Verified prior to writing this spec.** `drill/cli.py` defines `PROJECT_ROOT = Path(__file__).parent.parent`. After the move, `cli.py` lives at `evals/drill/cli.py`, so `PROJECT_ROOT` resolves to `evals/` and `PROJECT_ROOT.parent` resolves to the superteam repo root. That's the value `SUPERTEAM_ROOT` should take by default.

**YAML substitution audit.** Only the four `claude*.yaml` backend configs interpolate `${SUPERTEAM_ROOT}` into `args` (for the `--plugin-dir` flag); `codex.yaml` and `gemini.yaml` only list `SUPERTEAM_ROOT` in `required_env` (consumed by `engine.py:233` / `setup.py:25`'s `os.environ["SUPERTEAM_ROOT"]` lookups in pre/post-run hooks). The helper's `os.environ` mutation covers both code paths.

| File | Current | After |
|------|---------|-------|
| `drill/cli.py` | `load_dotenv(PROJECT_ROOT / ".env")` at module import; nothing about `SUPERTEAM_ROOT` | After `load_dotenv`, call new helper `_set_superteam_root_default()` that sets `os.environ["SUPERTEAM_ROOT"]` to `str(PROJECT_ROOT.parent)` if and only if not already set. Order: `load_dotenv` → set default → click group definitions. |
| `drill/engine.py:233`, `drill/setup.py:25` | Direct `os.environ["SUPERTEAM_ROOT"]` access (KeyError if unset) | Unchanged. The CLI startup hook guarantees the env var is set by the time the engine/setup execute. |
| `backends/claude*.yaml` (5 files) | `${SUPERTEAM_ROOT}` substituted in `args` for `--plugin-dir` | Unchanged. YAML substitution reads `os.environ` at backend-load time, which is after CLI startup. |
| `backends/codex.yaml`, `backends/gemini.yaml` | `SUPERTEAM_ROOT` in `required_env` only | Drop from `required_env` (the helper supplies it). `claude*.yaml` keep `required_env` for backward compat (env var works as override). |
| `evals/tests/test_backend.py` | Tests assert `SUPERTEAM_ROOT` is in `required_env` lists, plus path-resolution tests | Update tests to match the new contract: helper-supplied default, env override still works, `required_env` no longer required for codex/gemini. |
| `evals/README.md` | "export SUPERTEAM_ROOT=/path/to/superteam" | Drop the export line; note that the env var auto-defaults to the parent of `evals/`; mention the only required setup is `ANTHROPIC_API_KEY` (or `OPENAI_API_KEY` / Gemini auth). |
...
| `evals/lefthook.yml` | drill ships `lefthook.yml` defining `pre-commit: uv run ruff check && uv run ty check` | Move to `evals/lefthook.yml`. Either (a) install lefthook at the superteam root and have it federate to `evals/lefthook.yml`, or (b) document that contributors run `cd evals && lefthook run pre-commit` manually. **Decision in implementation: option (b) for simplicity** — superteam' top-level workflow doesn't change. |
...
**Top-level superteam files needing small additions:**

- `superteam/.gitignore`: add `evals/results/`, `evals/.venv/`, `evals/.env` (belt-and-suspenders; evals/.gitignore already covers these locally).
- `superteam/CLAUDE.md`: add a one-line pointer "Eval harness lives at `evals/` — see `evals/README.md`" so agents discover it.
- `superteam/docs/testing.md`: split into "## Plugin tests" (existing tests/ content, with the deleted-test references trimmed) and "## Skill behavior evals" (one-paragraph summary + pointer to `evals/`).
- `superteam/README.md`: add a single line in the Contributing section pointing at `evals/` for skill-behavior testing.
...
3. Update path defaults
   ├─ Add _set_superteam_root_default() helper to drill/cli.py
   ├─ Wire it after load_dotenv, before click group definition
   ├─ Update evals/README.md and evals/CLAUDE.md (drop SUPERTEAM_ROOT install step)
   ├─ Drop SUPERTEAM_ROOT from required_env in codex.yaml/gemini.yaml
...
6. Stale-reference scrub
   ├─ Subagent greps the superteam tree (excluding node_modules/, .venv/,
   │  evals/) for deleted file paths
   ├─ Search targets: docs/, docs/superteam/plans/, RELEASE-NOTES.md,
   ...
   └─ Historical references in docs/superteam/plans/*.md and
...
10. Push branch + open PR against dev
    └─ PR description includes: drill SHA pinned at copy, archival action
       ...
       obra/superteam/evals/"), per-deleted-file coverage receipts.
```
...
- `cd evals && uv sync` succeeds without `SUPERTEAM_ROOT` set.
...
- Grep for deleted file paths returns zero hits across living superteam docs (post step 6); historical refs in `docs/superteam/plans/*.md` and `RELEASE-NOTES.md` are annotated, not rewritten.
...
| Question | Decision |
|----------|----------|
| Where does drill live in superteam? | `evals/` (rename from drill); standalone repo archived as separate step |
