...
**Files:**
- Create: `docs/superteam/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md`
...
`docs/superteam/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md` with exactly these sections, filled from the artifacts:
...
- **Quote bank** — verbatim, minimum these six (pull more from the reply files if useful):
  ...
  - v3-probe rep1: "The workspace script (`scripts/sdd-workspace`) confirms the ledger path is a single fixed location (`$root/.superteam/sdd`), not plan-scoped, so it will collide across any two plans run in the same repo."
...
```bash
git add docs/superteam/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md
...
- Produces: `sdd-workspace PLAN_FILE` → prints `<repo-root>/.superteam/sdd/<plan-basename-without-.md>` (creates it; maintains `<repo-root>/.superteam/sdd/.gitignore` containing `*`). `task-brief PLAN_FILE N [OUTFILE]` → default OUTFILE `<workspace>/task-<N>-brief.md`. `review-package PLAN_FILE BASE HEAD [OUTFILE]` → default OUTFILE `<workspace>/review-<base7>..<head7>.diff`. Task 3's SKILL.md text names exactly these signatures.
...
PLAN
    ...
    if [[ "$dir_a" == "$repo/.superteam/sdd/plan-a" ]]; then
        pass "prints <repo-root>/.superteam/sdd/<plan-basename>"
    else
        fail "prints <repo-root>/.superteam/sdd/<plan-basename>"
        echo "    got: $dir_a"
    fi
    ...
    if [[ -f "$repo/.superteam/sdd/.gitignore" && "$(cat "$repo/.superteam/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created at .superteam/sdd/ with '*'"
    else
        fail "self-ignoring .gitignore created at .superteam/sdd/ with '*'"
    fi
    ...
    if [[ "$brief_path" == "$repo/.superteam/sdd/plan-a/task-1-brief.md" ]]; then
    ...
    case "$rp_path" in
        "$repo/.superteam/sdd/plan-a/review-"*.diff)
    ...
    if [[ "$wt_dir" == "$wt_root/.superteam/sdd/plan-a" && "$wt_dir" != "$dir_a" ]]; then
...
#
# One directory per plan (.superteam/sdd/<plan-basename>/) so a follow-up
...
# .gitignore at .superteam/sdd/ keeps every plan's workspace out of
...
# Default OUTFILE: <repo-root>/.superteam/sdd/<plan-basename>/review-<base7>..<head7>.diff
...
git commit -m "feat(sdd): plan-scoped workspace — one .superteam/sdd/<plan> dir per plan

sdd-workspace now requires the plan file and resolves
.superteam/sdd/<plan-basename>/; task-brief and review-package write
...
- At skill start, check for a ledger:
  `cat "$(git rev-parse --show-toplevel)/.superteam/sdd/progress.md"`. Tasks listed there
...
- Check for this plan's ledger at `<workspace>/progress.md`. If its first
  ...
  at the old flat path `.superteam/sdd/progress.md` — is another plan's
...
Old:
```
    "Dispatch final code reviewer subagent (../requesting-code-review/code-reviewer.md)" -> "Use superteam:finishing-a-development-branch";
```
New:
```
...
- Create: `docs/superteam/specs/2026-07-06-sdd-plan-scoped-workspace-eval-results.md`
- Delete: `docs/superteam/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md` (content folds into the results doc)
...
#   LAYOUT:   flat (released layout: .superteam/sdd/progress.md)
#             scoped (new layout: .superteam/sdd/<plan-basename>/progress.md,
...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superteam:subagent-driven-development.
...
case "$scenario/$layout" in
  s1/flat)
    mkdir -p .superteam/sdd
    plan_a_ledger_lines > .superteam/sdd/progress.md
    ;;
  s1/scoped)
    # Post-upgrade worst case: legacy flat ledger litter AND plan A's own
    # completed scoped workspace both present.
    mkdir -p .superteam/sdd/2026-07-01-widget-backend
    printf '*\n' > .superteam/sdd/.gitignore
    plan_a_ledger_lines > .superteam/sdd/progress.md
    {
      printf '# SDD ledger — plan: docs/plans/2026-07-01-widget-backend.md\n\n'
      plan_a_ledger_lines
    } > .superteam/sdd/2026-07-01-widget-backend/progress.md
    ;;
  s2/flat)
    mkdir -p .superteam/sdd
    plan_b_ledger_lines > .superteam/sdd/progress.md
    ;;
  s2/scoped)
    mkdir -p .superteam/sdd/2026-07-06-widget-export
    printf '*\n' > .superteam/sdd/.gitignore
    {
      printf '# SDD ledger — plan: docs/plans/2026-07-06-widget-export.md\n\n'
      plan_b_ledger_lines
    } > .superteam/sdd/2026-07-06-widget-export/progress.md
    ;;
  *)
...
Sanity-check one build: `bash "$EVAL_ROOT/make-fixture.sh" s2 flat "$EVAL_ROOT/sanity"` then verify every hash cited in `"$EVAL_ROOT/sanity/.superteam/sdd/progress.md"` resolves via `git -C "$EVAL_ROOT/sanity" cat-file -e <hash>` and that `git -C "$EVAL_ROOT/sanity" log --format='%an %ad' --date=short` shows two authors across two dates.
...
Create `docs/superteam/specs/2026-07-06-sdd-plan-scoped-workspace-eval-results.md` with exactly these sections (fill with real data):
...
coarse cost proxy. A rerunnable harness case belongs in superteam-evals
...
```bash
git rm -q docs/superteam/specs/2026-07-06-sdd-plan-scoped-workspace-eval-notes-red.md
git add docs/superteam/specs/2026-07-06-sdd-plan-scoped-workspace-eval-results.md
