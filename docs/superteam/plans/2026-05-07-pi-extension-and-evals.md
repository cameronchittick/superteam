...
Create `tests/pi/test-pi-extension.mjs` with tests that import `extensions/superteam.ts`, register fake Pi handlers, and assert:
- root `package.json` has `keywords` containing `pi-package`
- root `package.json` has `pi.skills: ["./skills"]`
- root `package.json` has `pi.extensions: ["./extensions/superteam.ts"]`
...
Expected: FAIL because `extensions/superteam.ts` does not exist and `package.json` lacks the `pi` manifest.
...
**Files:**
- Create: `skills/using-superteam/references/pi-tools.md`
- Modify: `tests/pi/test-pi-extension.mjs`

- [ ] **Step 1: Write failing test for Pi reference doc**

Add assertions that `skills/using-superteam/references/pi-tools.md` exists and documents mappings for `Skill`, `Task`, `TodoWrite`, and built-in tool names.
...
Create `skills/using-superteam/references/pi-tools.md` explaining Pi-native skills, optional `pi-subagents`, no canonical todo/tasklist plugin, and built-in lowercase tools.
...
- Pi backend command starts with `pi` and includes `-e ${SUPERTEAM_ROOT}`
...
Configure the backend to run `pi -e ${SUPERTEAM_ROOT}`, use permissive TUI readiness, `/quit` shutdown, and Pi session log location.
