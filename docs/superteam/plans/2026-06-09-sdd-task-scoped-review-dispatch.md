...
**Spec:** `docs/superteam/specs/2026-06-09-sdd-task-scoped-review-dispatch-design.md` — read it before starting. Decisions already settled there: full re-reviews stay; the two review stages stay separate; coordinator keeps model judgment; `requesting-code-review/` stays broad.
...
```
- [code-quality-reviewer-prompt.md](code-quality-reviewer-prompt.md) - Dispatch code quality reviewer subagent
- Final whole-branch review: use superteam:requesting-code-review's [code-reviewer.md](../requesting-code-review/code-reviewer.md)
```
...
```
- **superteam:requesting-code-review** - Code review template for reviewer subagents
```

with:

```
- **superteam:requesting-code-review** - Code review template for the final whole-branch review
```
...
Lives in the `evals/` **submodule** (separate repo, `superteam-evals`). Work on a branch there; the parent submodule-pointer bump happens at finishing time per `evals/CLAUDE.md`.
...
def scaffold_sdd_quality_defect_plan(workdir: Path) -> None:
    ...
    plans_dir = workdir / "docs" / "superteam" / "plans"
...
pre() {
    ...
    file-contains 'docs/superteam/plans/report-plan.md' 'formatAdminReport'
    file-contains 'docs/superteam/plans/report-plan.md' 'repeat\(40\)'
}

post() {
    skill-called superteam:subagent-driven-development
    tool-called Agent
...
Run: `grep -n "code-quality-reviewer" skills/using-superteam/references/antigravity-tools.md skills/using-superteam/references/gemini-tools.md`
...
- [ ] **Step 1: Baseline (skills as released on dev)** — from the main checkout (`/Users/jesse/git/superteam/superteam`, on dev), or any checkout without this branch's changes:

```bash
cd evals
export SUPERTEAM_ROOT=/Users/jesse/git/superteam/superteam
...
- [ ] **Step 2: After (this branch's skills)** — point `SUPERTEAM_ROOT` at this worktree:

```bash
cd evals
export SUPERTEAM_ROOT=/Users/jesse/git/superteam/superteam/.claude/worktrees/sdd-review-dispatch
uv run quorum run scenarios/sdd-rejects-extra-features --coding-agent claude
uv run quorum run scenarios/sdd-go-fractals --coding-agent claude
uv run quorum run scenarios/sdd-svelte-todo --coding-agent claude
uv run quorum run scenarios/spec-reviewer-catches-planted-flaws --coding-agent claude
...
After all tasks pass: the evals submodule commit needs to land in `superteam-evals` (PR to its `main`), then this branch bumps the `evals` submodule pointer — per `evals/CLAUDE.md`, the parent bump is part of propagation, not optional. Then use superteam:finishing-a-development-branch. PRs against superteam target `dev`.
