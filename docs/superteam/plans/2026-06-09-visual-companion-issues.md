...
- **Remote serving is a first-class scenario.** Superteam is general-purpose;
...
- Env var (e.g. `SUPERTEAM_VISUAL_COMPANION=off`) the skill is told to read —
  simplest, matches what the issue asks for, lives in `.zshrc`.
- A plugin-settings file (`.claude/superteam.local.md` frontmatter) — more
...
Today `--project-dir` writes session state to `<project>/.superteam/brainstorm/`
(`start-server.sh:80-84`) and the skill tells the user to gitignore it
(`visual-companion.md:58`). The ask is a `--state-dir` / `SUPERTEAM_STATE_DIR`
