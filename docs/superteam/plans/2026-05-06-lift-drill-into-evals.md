# Lift drill into superteam as `evals/` — implementation plan
...
**Goal:** Move the standalone `obra/drill` skill-compliance benchmark into superteam as a top-level `evals/` directory, delete redundant bash tests under `superteam/tests/` after per-file subagent verification of drill scenario coverage, and update top-level docs so contributors land on the new structure.

**Architecture:** Single PR against `dev` on a new branch `f/evals-lift`. Drill source is copied verbatim with explicit rsync excludes to keep `.git/`, `.venv/`, etc. out of the new dir. A small helper in `drill/cli.py` defaults `SUPERTEAM_ROOT` to the parent of the `evals/` directory, so contributors don't have to set the env var. Each bash-test deletion is gated by a subagent that compares the bash test's assertions to its claimed drill scenario's verify block. Historical references in plan docs and release notes are annotated, not rewritten.

**Tech Stack:** Python 3.11 + uv (drill's existing toolchain, unchanged); rsync; bash; git.

**Spec:** `docs/superteam/specs/2026-05-06-lift-drill-into-evals-design.md` — read this first.

**Drill source location:** `/Users/jesse/Documents/GitHub/superteam/drill/` (sibling to `superteam/`).
...
```bash
cd /Users/jesse/Documents/GitHub/superteam/superteam
git status --short
```
...
```bash
cd /Users/jesse/Documents/GitHub/superteam/drill
...
```bash
cd /Users/jesse/Documents/GitHub/superteam/superteam
test -d /Users/jesse/Documents/GitHub/superteam/drill && echo "drill source: OK"
...
rsync -a \
  ...
  --exclude='*.pyc' \
  /Users/jesse/Documents/GitHub/superteam/drill/ \
  evals/
```
...
rsync of obra/drill@$DRILL_SHA into superteam/evals/, excluding
...
while read -r f; do
  ...
  sha2=$(shasum -a 256 "/Users/jesse/Documents/GitHub/superteam/superteam/evals/${f#./}" | cut -d' ' -f1)
  if [ "$sha1" != "$sha2" ]; then
...
```bash
cd /Users/jesse/Documents/GitHub/superteam/superteam/evals
uv sync
```
...
Expected: starts with scenario names. (Will likely error or warn about missing SUPERTEAM_ROOT — that's fine, fixed in next task.)
...
/Users/jesse/Documents/GitHub/superteam/drill into
/Users/jesse/Documents/GitHub/superteam/superteam/evals.

Verify:

1. The lift commit message records the SHA reported by:
  cd /Users/jesse/Documents/GitHub/superteam/drill && git rev-parse HEAD
...
## Task 5: Add `SUPERTEAM_ROOT` default helper
...
```bash
sed -n '1,20p' /Users/jesse/Documents/GitHub/superteam/superteam/evals/drill/cli.py
```
...
```python
def test_set_superteam_root_default_when_unset(monkeypatch, tmp_path):
    """When SUPERTEAM_ROOT is unset, helper sets it to PROJECT_ROOT.parent."""
    monkeypatch.delenv("SUPERTEAM_ROOT", raising=False)
    from drill.cli import _set_superteam_root_default, PROJECT_ROOT

    _set_superteam_root_default()

    import os
    assert os.environ["SUPERTEAM_ROOT"] == str(PROJECT_ROOT.parent)


def test_set_superteam_root_default_respects_existing(monkeypatch):
    """When SUPERTEAM_ROOT is already set, helper does not override."""
    monkeypatch.setenv("SUPERTEAM_ROOT", "/custom/path")
    from drill.cli import _set_superteam_root_default

    _set_superteam_root_default()

    import os
    assert os.environ["SUPERTEAM_ROOT"] == "/custom/path"
```
...
```bash
cd /Users/jesse/Documents/GitHub/superteam/superteam/evals
uv run pytest tests/test_cli.py -k set_superteam_root_default -v
```

Expected: 2 tests fail with `AttributeError: module 'drill.cli' has no attribute '_set_superteam_root_default'`.

- [ ] **Step 4: Add the helper to cli.py**

Edit `/Users/jesse/Documents/GitHub/superteam/superteam/evals/drill/cli.py`. Replace lines 1–14 with:
...
def _set_superteam_root_default() -> None:
    """Default SUPERTEAM_ROOT to the parent of evals/ if not already set.

    Drill historically required contributors to export SUPERTEAM_ROOT
    pointing at the superteam checkout. After lifting drill into
    superteam/evals/, the parent of PROJECT_ROOT is always the
    superteam root, so we can supply this default automatically.

    Existing SUPERTEAM_ROOT environment values are respected as overrides.
    """
    os.environ.setdefault("SUPERTEAM_ROOT", str(PROJECT_ROOT.parent))


_set_superteam_root_default()
```

The bottom-of-module call to `_set_superteam_root_default()` runs at import time, immediately after `load_dotenv()`. This ensures both `engine.py` and `setup.py` (which read `os.environ["SUPERTEAM_ROOT"]` directly) and the YAML interpolation (which reads `os.environ` when the backend YAML is loaded) all see the value.
...
git commit -m "evals: default SUPERTEAM_ROOT to parent of evals/ if unset

Adds _set_superteam_root_default() to drill/cli.py, called at
module import after load_dotenv(). PROJECT_ROOT resolves to evals/
post-lift; its parent is the superteam repo root, which is the
correct value for SUPERTEAM_ROOT.
...
**Files:**
- Modify: `evals/backends/codex.yaml` (drop `SUPERTEAM_ROOT` from `required_env`)
- Modify: `evals/backends/gemini.yaml` (drop `SUPERTEAM_ROOT` from `required_env`)

The five `claude*.yaml` backend configs interpolate `${SUPERTEAM_ROOT}` into `args` for the `--plugin-dir` flag — they keep `SUPERTEAM_ROOT` in `required_env` because the interpolation needs it. The codex/gemini configs only listed it for engine.py/setup.py's `os.environ` reads, which the helper now satisfies.

- [ ] **Step 1: Confirm current state**

```bash
grep -A3 'required_env:' /Users/jesse/Documents/GitHub/superteam/superteam/evals/backends/codex.yaml
grep -A2 'required_env:' /Users/jesse/Documents/GitHub/superteam/superteam/evals/backends/gemini.yaml
```

Expected outputs include `- SUPERTEAM_ROOT` lines.

- [ ] **Step 2: Read codex.yaml fully**

```bash
cat /Users/jesse/Documents/GitHub/superteam/superteam/evals/backends/codex.yaml
```

- [ ] **Step 3: Edit codex.yaml — drop the `- SUPERTEAM_ROOT` line under `required_env`**
...
- [ ] **Step 4: Edit gemini.yaml — drop the `- SUPERTEAM_ROOT` line under `required_env`**
...
git commit -m "evals: drop SUPERTEAM_ROOT from codex/gemini required_env

These backends only read SUPERTEAM_ROOT via engine.py/setup.py's
os.environ access, which the new cli.py default helper supplies
automatically. claude*.yaml keep SUPERTEAM_ROOT in required_env
because they interpolate \${SUPERTEAM_ROOT} into --plugin-dir args."
```
...
For tests that assert `SUPERTEAM_ROOT` membership in `codex.yaml`'s or `gemini.yaml`'s `required_env`: invert the assertion to confirm absence. Example:

```python
# Before:
def test_codex_requires_superteam_root():
    backend = load_backend("codex")
    assert "SUPERTEAM_ROOT" in backend.required_env

# After:
def test_codex_does_not_require_superteam_root():
    """codex.yaml dropped SUPERTEAM_ROOT from required_env;
    the cli.py helper supplies the default."""
    backend = load_backend("codex")
    assert "SUPERTEAM_ROOT" not in backend.required_env
```
...
**Files:**
- Modify: `evals/README.md` (drop SUPERTEAM_ROOT setup step)
- Modify: `evals/CLAUDE.md` (drop SUPERTEAM_ROOT setup step)
...
```bash
export SUPERTEAM_ROOT=/path/to/superteam
...
`SUPERTEAM_ROOT` defaults to the parent of `evals/` (the superteam repo root) and only needs to be set if you're running drill against a different superteam checkout.
```
...
```
SUPERTEAM_ROOT=/path/to/superteam
...
`SUPERTEAM_ROOT` defaults to the parent of `evals/` (the superteam repo root). Override only if running drill against a different superteam checkout.
```
...
git commit -m "evals: drop SUPERTEAM_ROOT setup step from README/CLAUDE
...
Expected: scenario list, no error about missing SUPERTEAM_ROOT.
...
If FAIL, debug before continuing. The path-defaults change is the most likely culprit; check that the helper actually fired by adding a `print(os.environ["SUPERTEAM_ROOT"])` after the helper call temporarily.
...
```bash
ls /Users/jesse/Documents/GitHub/superteam/superteam/tests/skill-triggering/prompts/
```
...
```bash
for f in /Users/jesse/Documents/GitHub/superteam/superteam/tests/explicit-skill-requests/*.sh /Users/jesse/Documents/GitHub/superteam/superteam/tests/explicit-skill-requests/prompts/*.txt; do
  echo "=== $f ==="
  cat "$f" | head -30
done
```
...
```bash
ls /Users/jesse/Documents/GitHub/superteam/superteam/evals/fixtures/sdd-go-fractals/
ls /Users/jesse/Documents/GitHub/superteam/superteam/evals/fixtures/sdd-svelte-todo/
```
...
```bash
cat /Users/jesse/Documents/GitHub/superteam/superteam/tests/claude-code/test-subagent-driven-development.sh
```
...
- Annotate (do not rewrite): `RELEASE-NOTES.md`, `docs/superteam/plans/*.md`
...
| `docs/superteam/plans/*.md` | Annotate, don't rewrite (dated artifact) |
...
For each `RELEASE-NOTES.md` or `docs/superteam/plans/*.md` hit, add an inline annotation at the *first* hit per file:
...
```
Working directory: /Users/jesse/Documents/GitHub/superteam/superteam
...
Search the entire superteam tree (excluding evals/, node_modules/,
...
and docs/superteam/plans/*.md are annotated as dated artifacts,
not rewritten."
```
...
Open `/Users/jesse/Documents/GitHub/superteam/superteam/docs/testing.md` and replace the file content with this structure (preserve the existing Plugin-test details where applicable):

```markdown
# Testing Superteam

Superteam has two distinct kinds of tests, each in its own directory:
...
Open `/Users/jesse/Documents/GitHub/superteam/superteam/.gitignore` and add at the bottom:
...
```bash
cd /Users/jesse/Documents/GitHub/superteam/superteam/tests/brainstorm-server
...
**Branch:** f/evals-lift, in /Users/jesse/Documents/GitHub/superteam/superteam
**Base:** dev (currently b4363df)
**Spec:** docs/superteam/specs/2026-05-06-lift-drill-into-evals-design.md

This branch lifts the obra/drill repo into superteam/evals/ and
...
3. Does the SUPERTEAM_ROOT helper actually default correctly when
   the env var is unset? (cd evals && unset SUPERTEAM_ROOT && uv
...
gh pr create \
  ...
  --head f/evals-lift \
  --reviewer arittr \
  --title "Lift drill into superteam as evals/ harness" \
  --body "$(cat <<'EOF'
## What problem are you trying to solve?

Drill — the standalone Python skill-compliance benchmark at obra/drill — is already the de facto eval harness for superteam. The PRI-1397 commit series lifted ~22 bash tests into drill scenarios, and the most recent superteam commit (a2292c5) explicitly removed a redundant bash test with the message "replaced by drill behavioral coverage". Drill is a sibling repo today, requiring contributors to clone two checkouts and set SUPERTEAM_ROOT manually. This PR completes the migration: drill becomes superteam/evals/.

## What does this PR change?

- Lifts the obra/drill repo into superteam as `evals/`, with explicit rsync excludes (.git, .venv, results, .env, __pycache__, *.egg-info, .private-journal). The lift commit records the source SHA.
- Adds a `_set_superteam_root_default()` helper to drill/cli.py so SUPERTEAM_ROOT defaults to the parent of evals/ — no manual env-var setup.
- Drops SUPERTEAM_ROOT from required_env in codex.yaml/gemini.yaml (the helper supplies it). Claude*.yaml keep it because they interpolate ${SUPERTEAM_ROOT} into --plugin-dir args.
...
Yes. Cross-runtime evaluation is core to superteam, the migration to drill scenarios was already underway in this repo, and the eval harness needs to be discoverable in-tree to be findable.

## What alternatives did you consider?

- Vendored copy + sync script (drill repo continues independently). Rejected: divergence risk; single-source-of-truth wins.
- git subtree merge (preserves drill history in-tree). Rejected: superteam' git history grows by 50+ commits, the merge commit is ugly, subtrees are operationally heavy.
- Keep drill as a sibling repo and just polish docs. Rejected: doesn't solve the discoverability problem.

## Does this PR contain multiple unrelated changes?

No — every change supports "drill is now evals/ inside superteam". Multiple commits for atomicity (verbatim copy, env helper, YAML updates, docs) but one direction.

## Existing PRs

- [x] I have reviewed all open AND closed PRs for duplicates or prior art
- Related PRs: #1486 (obra/superteam cross-platform PR — independent; no shared file changes besides README, which has no overlap)
...
- Initial prompt: see linked spec (`docs/superteam/specs/2026-05-06-lift-drill-into-evals-design.md`).
- Drill's own pytest suite passes.
- One drill scenario re-run from the new location end-to-end (proves the SUPERTEAM_ROOT default works).
...
1. Archive obra/drill on GitHub (mark read-only, add README pointer to obra/superteam/evals/).
...
- [ ] `cd evals && unset SUPERTEAM_ROOT && uv run pytest` passes
- [ ] `cd evals && unset SUPERTEAM_ROOT && uv run drill list` returns scenarios
- [ ] `cd evals && unset SUPERTEAM_ROOT && uv run drill run triggering-test-driven-development -b claude` passes
- [ ] `tests/brainstorm-server/server.test.js` still passes (regression gate for non-LLM tests)
- [ ] `git diff dev..HEAD docs/superteam/plans/2026-04-06-worktree-rototill.md docs/superteam/plans/2026-03-23-codex-app-compatibility.md RELEASE-NOTES.md` shows annotations only, no path rewrites
