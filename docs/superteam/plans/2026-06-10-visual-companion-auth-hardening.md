...
```bash
cd /Users/drewritter/prime-rad/superteam/tests/brainstorm-server
node auth.test.js
```
...
```bash
cd /Users/drewritter/prime-rad/superteam
...
```bash
cd /Users/drewritter/prime-rad/superteam
git check-ignore .superteam/brainstorm/.last-token || true
```
...
```bash
cd /Users/drewritter/prime-rad/superteam
git check-ignore .superteam/brainstorm/.last-token
```

Expected output:

```text
.superteam/brainstorm/.last-token
```
...
```bash
node /tmp/superteam-pr1720-security-drewritter/probe-pr1720.cjs
```
