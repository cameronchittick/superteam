...
**Architecture:** Two repos. The `superteam` repo (branch `sdd-fix-loop-redesign`, already created; spec committed) gets the skill restructure: one new prompt template, two template edits, one reference edit, and the SKILL.md rewrite whose full text is in Task 3. The `superteam-evals` repo (`evals/` checkout; create branch `sdd-fix-loop-scenarios` off `main`) gets two seeded-ledger fixture helpers and three scenarios, then a live before/after campaign.
...
**Design spec:** `docs/superteam/specs/2026-07-15-sdd-fix-loop-redesign-design.md`. Read it before starting any task.
...
- **Commit discipline:** superteam commits on `sdd-fix-loop-redesign`; evals commits on `sdd-fix-loop-scenarios` (separate repo — `cd evals` first). Never commit one repo's work from the other.
- **Static gates before any live run:** `bun run check` and `bun run quorum check` pass in `evals/`.
- **Live runs are trusted-maintainer operations** — they need `SUPERTEAM_ROOT`, an `ANTHROPIC_API_KEY`, and cost real money (~$3–15 per SDD run). Task 8 marks them explicitly.
...
- Modify: `skills/using-superteam/references/codex-tools.md` — implementer close timing (Task 2)
- Modify: `skills/subagent-driven-development/SKILL.md` — full restructure (Task 3)

**superteam-evals repo (`evals/`):**
...
- Modify: `skills/using-superteam/references/codex-tools.md` (subagent close timing)
...
```bash
git add skills/subagent-driven-development/implementer-prompt.md skills/subagent-driven-development/task-reviewer-prompt.md skills/using-superteam/references/codex-tools.md
...
digraph process {
    ...
    "Final findings? ONE fix dispatch, one scoped re-review, adjudicate residuals" -> "Use superteam:finishing-a-development-branch";
}
```
...
- At skill start, check for a ledger:
  `cat "$(git rev-parse --show-toplevel)/.superteam/sdd/progress.md"`. Tasks with
...
- Produces: registry names `scaffold_sdd_midloop_parked` and `scaffold_sdd_midloop_structural` (Tasks 6–7 setup.sh call these); fixture repo with `docs/superteam/plans/metrics-plan.md`, Tasks 1–2 implemented and committed, `.superteam/sdd/progress.md` seeded at fix round 5/5 with one open finding, and (parked variant) `npm test` green.
  ...
  test('scaffoldSddMidloopParked seeds a round-5 ledger with real SHAs and green tests', () => {
    ...
    try {
      ...
      const ledger = readFileSync(
        join(dir, '.superteam/sdd/progress.md'),
        'utf8',
      );
  ...
  test('scaffoldSddMidloopStructural seeds a plan-contradiction finding', () => {
    ...
    try {
      ...
      const plan = readFileSync(
        join(dir, 'docs/superteam/plans/metrics-plan.md'),
        'utf8',
      );
...
function scaffoldSddMidloop(ctx: HelperContext, opts: MidloopOptions): void {
  ...
  writeFixtureFile(ctx.workdir, '.gitignore', '.superteam/\n');
  writeFixtureFile(
    ctx.workdir,
    'docs/superteam/plans/metrics-plan.md',
  ...
  const ledger = [
    '# SDD Progress Ledger',
    'Plan: docs/superteam/plans/metrics-plan.md',
    `Task 1: complete (commits ${base}..${task1Head}, review clean)`,
    `Task 2: implementer DONE (commits ${task2Base}..${task2Head})`,
    ...roundLines,
    '',
  ].join('\n');
  writeFixtureFile(ctx.workdir, '.superteam/sdd/progress.md', ledger);

  writeFixtureFile(
    ctx.workdir,
    '.superteam/sdd/task-2-report.md',
    `# Task 2 Report
...
You have a small plan at docs/superteam/plans/report-plan.md — two report
...
"I have a small plan at docs/superteam/plans/report-plan.md — two report
formatting functions. Use the superteam:subagent-driven-development skill
...
- A `Skill` invocation naming `superteam:subagent-driven-development`
...
pre() {
    ...
    git-branch main
    requires-tool npm
    file-exists 'docs/superteam/plans/report-plan.md'
    file-contains 'docs/superteam/plans/report-plan.md' 'asserts nothing'
}

post() {
    check-transcript skill-called superteam:subagent-driven-development
...
at docs/superteam/plans/metrics-plan.md, mid-execution: Task 1 is
...
progress ledger at .superteam/sdd/progress.md records all of this. You
are spec-aware — name the skill.

Tell the agent:

"I had to restart our session. We were executing
docs/superteam/plans/metrics-plan.md with the
superteam:subagent-driven-development skill — the progress ledger is at
.superteam/sdd/progress.md. Pick up where we left off and finish the
plan."
...
pre() {
    ...
    git-branch main
    requires-tool npm
    file-exists '.superteam/sdd/progress.md'
    file-contains '.superteam/sdd/progress.md' 'fix round 5/5'
...
post() {
    check-transcript skill-called superteam:subagent-driven-development
    check-transcript tool-called Agent
    file-contains '.superteam/sdd/progress.md' 'Task 2: parked —'
    file-contains '.superteam/sdd/progress.md' 'ruling:'
    file-contains '.superteam/sdd/progress.md' 'Task 3: complete'
    not file-contains '.superteam/sdd/progress.md' 'fix round 6'
...
You are resuming an interrupted SDD session. The repo has a three-task plan
at docs/superteam/plans/metrics-plan.md, mid-execution: Task 1 complete,
...
.superteam/sdd/progress.md records the exhausted loop. You are
...
pre() {
    ...
    file-exists '.superteam/sdd/progress.md'
    file-contains '.superteam/sdd/progress.md' 'fix round 5/5'
    file-contains '.superteam/sdd/progress.md' 'milliseconds'
    not file-exists 'src/summary.js'
}
...
post() {
    check-transcript skill-called superteam:subagent-driven-development
    not file-exists 'src/summary.js'
    not file-contains '.superteam/sdd/progress.md' 'Task 3: complete'
    not file-contains '.superteam/sdd/progress.md' 'fix round 6'
}
```
...
`SUPERTEAM_ROOT`, and cost real money (estimate: 3 new scenarios × 2
...
**Interfaces:**
- Consumes: everything from Tasks 1–7; a second superteam checkout pinned to `dev` for baselines.
- Produces: verdicts for the PR's before/after evidence.

- [ ] **Step 1: Prepare the two SUPERTEAM_ROOT checkouts**

```bash
git -C /Users/jesse/git/superteam-workspace/superteam worktree add /tmp/superteam-baseline dev
export BASELINE_ROOT=/tmp/superteam-baseline
export REDESIGN_ROOT=/Users/jesse/git/superteam-workspace/superteam   # on sdd-fix-loop-redesign
```
...
```bash
cd evals
SUPERTEAM_ROOT="$BASELINE_ROOT" bun run quorum run scenarios/sdd-fix-loop-resumes-implementer --coding-agent claude
SUPERTEAM_ROOT="$BASELINE_ROOT" bun run quorum run scenarios/sdd-breaker-adjudicates-at-cap --coding-agent claude
SUPERTEAM_ROOT="$BASELINE_ROOT" bun run quorum run scenarios/sdd-breaker-structural-blocks --coding-agent claude
bun run quorum show
```
...
```bash
cd evals
SUPERTEAM_ROOT="$REDESIGN_ROOT" bun run quorum run scenarios/sdd-fix-loop-resumes-implementer --coding-agent claude
SUPERTEAM_ROOT="$REDESIGN_ROOT" bun run quorum run scenarios/sdd-breaker-adjudicates-at-cap --coding-agent claude
SUPERTEAM_ROOT="$REDESIGN_ROOT" bun run quorum run scenarios/sdd-breaker-structural-blocks --coding-agent claude
bun run quorum show
```

Expected: all three PASS. Triage any non-pass with
`evals/docs/superteam/skills/triaging-a-failing-eval.md` before touching
...
for s in sdd-quality-reviewer-catches-planted-defect sdd-rejects-extra-features sdd-escalates-broken-plan sdd-spec-constraint-preserved; do
  SUPERTEAM_ROOT="$REDESIGN_ROOT" bun run quorum run "scenarios/$s" --coding-agent claude
done
bun run quorum show
```
...
git -C /Users/jesse/git/superteam-workspace/superteam worktree remove /tmp/superteam-baseline
```

- [ ] **Step 7: Hand off**

Both branches ready: `sdd-fix-loop-redesign` (superteam) and
`sdd-fix-loop-scenarios` (evals). Use superteam:finishing-a-development-branch
in each repo. The superteam PR carries the before/after verdicts from the
